...
import { Sun, Moon, Menu, X, Github, MessageSquare, Heart, Terminal } from 'lucide-react';
...
export default function Layout({ children, lang: langProp, activePath }: LayoutProps) {
  ...
  const defaultLang = location.startsWith('/ar') ? 'ar' : 'en';
  ...
  const handleLangToggle = () => {
    if (lang === 'ar') {
      const newPath = location.replace(/^\/ar/, '') || '/';
      setLocation(newPath);
    } else {
      const newPath = `/ar${location === '/' ? '' : location}`;
      setLocation(newPath);
    }
  };
    ...
    >
      ...
      <header className={`fixed top-0 inset-x-0 z-50 border-b border-[#008a2f]/20 ${isDark ? 'bg-black/60' : 'bg-white/60'} backdrop-blur-md`}>
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <Link href={lang === 'ar' ? '/ar' : '/'} className="flex items-center gap-3 group">
            <img
            ...
            <div>
              ...
              <ul className="space-y-2">
                {navItems.slice(0, 4).map(item => (
                  <li key={item.href}>
                    <Link href={item.href} className={`text-sm ${isDark ? 'text-gray-400 hover:text-[#008a2f]' : 'text-gray-500 hover:text-[#008a2f]'} transition-colors`}>
                      {item.label}
                    </Link>
                  </li>
                ))}
...
export function SectionHeading({ title, subtitle, isDark = true, center = true, lang = 'en' }: { title: string; subtitle?: string; isDark?: boolean; center?: boolean; lang?: Lang }) {
  const isRtl = lang === 'ar';
  return (
    <div className={`mb-12 ${center ? 'text-center' : ''}`} data-testid={`heading-${title}`}>
      <h2 className="text-3xl md:text-4xl font-bold uppercase tracking-widest mb-4" style={{ fontFamily: isRtl ? "'Almarai', sans-serif" : 'var(--font-mono)' }}>
...
export function PrimaryButton({ children, href, className = '', ...props }: { children: ReactNode; href?: string; className?: string; [key: string]: any }) {
...
export function SecondaryButton({ children, href, isDark = true, className = '', ...props }: { children: ReactNode; href?: string; isDark?: boolean; className?: string; [key: string]: any }) {
